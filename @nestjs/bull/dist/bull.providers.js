...
function createQueueOptionProviders(options) {
    const providers = options.map((option) => {
        const optionalSharedConfigHolder = (0, bull_shared_1.createConditionalDepHolder)((0, bull_shared_1.getSharedConfigToken)(option.configKey));
        return [
            optionalSharedConfigHolder,
            {
                provide: (0, bull_shared_1.getQueueOptionsToken)(option.name),
...
function createQueueProviders(options) {
    return options.map((option) => ({
        ...
        inject: [(0, bull_shared_1.getQueueOptionsToken)(option.name)],
    }));
}
